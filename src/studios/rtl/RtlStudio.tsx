...
export function RtlStudio() {
  ...
  const tab = params.get("tab") ?? "transfer";
  ...
  return (
    <StudioFrame icon="project" title="Register Transfer" description="Move a value from registers through the ALU and back. A shared bus accepts one driver." tabs={[{ id: "transfer", label: "Transfer" }, { id: "bus", label: "Bus" }, { id: "word", label: "Control Word" }]} tab={tab} onTab={(id) => setParams({ tab: id })} guide={["R3 <- R1 + R2 reads both sources and writes the sum.", "Two enabled drivers make the bus X.", "The control word is the bundle of those choices."]} takeaways={["Only the operators + - & | ^ are accepted.", "A move copies one register and skips the ALU.", "Contention is invalid data, not a third numeric value."]}>
