...
import { TrafficHop } from "../animation/phase3Views";
...
export function TrafficLab() {
  ...
  const lastHop = visible.at(-1);
  const hint = play.cycle === 0
    ? "Step the trace. Messages so far counts only events you have reached."
    : (lastHop?.dataBytes ?? 0) > 0
      ? `This step carries ${lastHop?.dataBytes} data bytes. A control message is counted separately.`
      : presetId === "ping"
        ? "Ownership is moving between cores."
        : presetId === "false"
          ? "These writes touch different variables on one line."
          : "Compare Messages so far with Private data after you Reset.";
  const reading = `Messages ${shown.messages}. Estimated bytes ${shown.bytes}.`;
  const add = () => {
  ...
  return (
    <LabChrome lab="coherence-traffic" kicker="Labs > Lab 21" title="Lab 21 — Cache-Coherence Traffic Analyzer" subtitle="Inspect and quantify coherence messages generated by multicore workloads." badge="RISC-V (5-Stage Pipeline)" hint={hint} reading={reading}>
      <div className="vl-cards three">
        ...
        <article><h2>Traffic</h2><p>Sharing tells a story about who had the line and who had to give it up. A private trace stays quiet. A ping-pong of writes moves ownership.</p>
          <TrafficHop message={visible.at(-1)?.message ?? ""} bytes={visible.at(-1)?.dataBytes ?? 0} from={visible.at(-1)?.source ?? ""} to={visible.at(-1)?.destination ?? ""} live={visible.at(-1)?.cycle === play.cycle} speed={play.speed} cycle={play.cycle} />
        </article>
      </div>
      <div className="vl-cards three">
        <article>
        ...
        <article className={guideFocus === "counters" ? "aca-guide-on" : undefined}>
      ...
      <div className="vl-cards three">
        <article className={guideFocus === "log" ? "aca-guide-on" : undefined}>
          <h2>Event Log</h2>
          {logOn ? (
            <table>
        ...
        <article>
          ...
          <Transport playing={play.playing} onPlay={() => { if (!auto) { play.setCycle((value) => Math.min(lastCycle, value + 1)); return; } play.setPlaying((value) => !value); }} onStep={() => play.setCycle((value) => Math.min(lastCycle, value + 1))} onBack={() => play.setCycle((value) => Math.max(0, value - 1))} onReset={() => { setGuideFocus(""); play.reset(); }} speed={play.speed} onSpeed={play.setSpeed} />
        </article>
        <article>
