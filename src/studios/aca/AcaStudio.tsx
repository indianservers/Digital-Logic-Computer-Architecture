...
import { DataHazardsLab } from "./labs/HazardLab";
...
export function AcaStudio() {
  ...
  if (lab.slug === "data-hazards") return <DataHazardsLab />;
  if (lab.slug === "scoreboard") return <ScoreboardLab />;
  if (lab.slug === "tomasulo") return <TomasuloLab />;
  if (lab.slug === "register-renaming") return <RenameLab />;
  if (lab.slug === "reorder-buffer") return <OooLab />;
  if (lab.slug === "branch-predictor") return <PredictorLab />;
  if (lab.slug === "correlating-predictor") return <CorrelateLab />;
  if (lab.slug === "branch-target-buffer") return <BtbLab />;
  if (lab.slug === "tournament-predictor") return <TournamentLab />;
  if (lab.slug === "speculative-execution") return <SpeculateLab />;
  if (lab.slug === "superscalar") return <SuperscalarLab />;
  if (lab.slug === "issue-queue") return <IssueQueueLab />;
  if (lab.slug === "physical-register-file") return <PrfLab />;
  if (lab.slug === "load-store-queue") return <LsqLab />;
  if (lab.slug === "memory-disambiguation") return <DisambigLab />;
  if (lab.slug === "execution-ports") return <PortLab />;
  if (lab.slug === "mesi") return <MesiLab />;
  if (lab.slug === "moesi") return <MoesiLab />;
  if (lab.slug === "directory-coherence") return <DirectoryLab />;
  if (lab.slug === "false-sharing") return <FalseShareLab />;
  if (lab.slug === "coherence-traffic") return <TrafficLab />;
  if (lab.slug === "snooping-vs-directory") return <SnoopLab />;
  if (lab.slug === "mshr") return <MshrLab />;
  if (lab.slug === "prefetching") return <PrefetchLab />;
  if (lab.slug === "dram-controller") return <DramLab />;
  if (lab.slug === "memory-consistency") return <ConsistencyLab />;
  if (lab.slug === "atomic-operations") return <SyncLab />;
  if (lab.slug === "amdahl") return <AmdahlLab />;
  if (lab.slug === "roofline") return <RooflineLab />;
  if (lab.slug === "cpi-ipc") return <CpiLab />;
  if (lab.slug === "performance-counters") return <CounterLab />;
